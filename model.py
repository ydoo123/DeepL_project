...
class PyramidNet(nn.Module):
    def __init__(
        ...
        if self.dataset in ["cifar10", "cifar100", "svhn"]:
            ...
            self.ps_shakedrop = [
                1.0 - (1.0 - (self.final_shake_p / (3 * n)) * (i + 1))
                for i in range(3 * n)
            ]
            ...
            if layers.get(depth) is None:
                ...
                print(
                    "=> the layer configuration for each stage is set to", layers[depth]
                )
            ...
            self.layer2 = self.pyramidal_make_layer(
                blocks[depth], layers[depth][1], stride=2
            )
            self.layer3 = self.pyramidal_make_layer(
                blocks[depth], layers[depth][2], stride=2
            )
            self.layer4 = self.pyramidal_make_layer(
                blocks[depth], layers[depth][3], stride=2
            )
